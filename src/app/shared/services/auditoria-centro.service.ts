...
export class AuditoriaCentroService {
  postString: string = '';
  constructor(private globalService: GlobalService, private http: HttpClient) { }

  getAllAuditorias(page): Observable<any> {
    const options =  { params: new HttpParams().set('page', page ) } ;
    return this.http.get(this.globalService.apiHost + 'auditoria-interna-centro', options)
  ...
  getV3Auditorias(dateFrom, hourFrom, dateTo, hourTo, pageIndex, pageSize): Observable<any> {
    ...
    return this.http.get(
      this.globalService.apiHost + "auditoria-interna/filtrar-centro",options
    );
  }
  ...
  getV3Excel(dateFrom, hourFrom, dateTo, hourTo): Observable<any> {
    ...
    return this.http.get(
      this.globalService.apiHost + "auditoria-interna/exportar-centro",options
    );
  }
