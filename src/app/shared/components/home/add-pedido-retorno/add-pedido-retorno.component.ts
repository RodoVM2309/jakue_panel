...
import {
  ...
  NativeDateAdapter, DateAdapter, MAT_DATE_FORMATS, MAT_DATE_LOCALE, MAT_DIALOG_DATA
...
@Component({
  selector: 'app-add-pedido-retorno',
  templateUrl: './add-pedido-retorno.component.html',
  styleUrls: ['./add-pedido-retorno.component.scss'],
  providers: [
    {
...
export class AddPedidoRetornoComponent implements OnInit {
  ...
  constructor(private centrosService: CentrosService, private origenesService: OrigenesService,private destinosService: DestinosService,
    ...
    public router: Router, public dialogLocaRef: MatDialogRef<AddPedidoRetornoComponent>, ) { }
  ...
  ngOnInit() {
    ...
    this.ocultarcampos_dador = (localStorage.getItem('clienteMuvin') === '2') ? false : true;
    ...
    this.addPedidoForm = new FormGroup({
      quantity: new FormControl(this.valorQuantity, [Validators.required, Validators.min(1)]),
      ...
      selectedProducto: new FormControl(this.busqueda.id_producto != undefined ? this.busqueda.id_producto : '', [Validators.required]),
      ...
      difundido: (this.tipocentro !== '2') ? new FormControl('') : new FormControl('', [Validators.required]),
  ...
  getListaCentro() {
    this.getItemSub = this.centrosService.getAllListaCentro2(this.tipoturneada)
      .subscribe(data => {
        ...
        this.mostrarTurneada =  this.tipoturneada>0 && data.data.length>0 ?true:false ;
      });
  }
  ...
  private _filter(descripcion: string): Origen[] {
    ...
    return this.origenes.filter(option => option.descripcion.toLowerCase().indexOf(filterValue) >= 0);
  }
  ...
  private _filterd(nombre_cliente: string): Dador[] {
    ...
    return this.dadores.filter(option_c => option_c.nombre_cliente.toLowerCase().indexOf(filterValue_c) >= 0);
  }
  ...
  private _filterdes(descripcion: string): Destino[] {
    ...
    return this.destinos.filter(option_d => option_d.descripcion.toLowerCase().indexOf(filterValue_d) >= 0);
  }
  ...
  getItemsOrigen() {
    this.getItemSub = this.nomencladoresService.getAllDestinosSelect()
      .subscribe(data => {
        ...
        this.filteredOptions = this.addPedidoForm.controls['selectedOrigen'].valueChanges
          .pipe(
            startWith<string | Origen>(''),
            map(value => typeof value === 'string' ? value : value.descripcion),
            map(descripcion => descripcion ? this._filter(descripcion) : this.origenes.slice())
          );
      })
  }
  getItemsDestino() {
    this.getItemSub = this.nomencladoresService.getAllOrigenesSelect()
      ...
      .subscribe(data => {
        ...
        this.filteredOptionsDestino = this.addPedidoForm.controls['selectedDestino'].valueChanges
          .pipe(
            startWith<string | Destino>(''),
            map(value => typeof value === 'string' ? value : value.descripcion),
            map(descripcion => descripcion ? this._filterdes(descripcion) : this.destinos.slice())
          );
      });
  }
  ...
  getItemsDador() {
    this.getItemSub = this.nomencladoresService.getAllDadores()
      .subscribe(data => {
        this.dadores = data.data;
        this.filteredselectedDador = this.addPedidoForm.controls['selectedDador'].valueChanges
          .pipe(
            startWith<string | Dador>(''),
            map(value => typeof value === 'string' ? value : value.nombre_cliente),
            map(nombre_cliente => nombre_cliente ? this._filterd(nombre_cliente) : this.dadores.slice())
          );
      })
  }
  ...
  postPedido() {
    if (this.f.selectedDestino.value.id===undefined || this.f.selectedDestino.value.id===null)
    this.errorService.confirm({ message: 'No exite destino!: Revise'});
    else {
      if (this.f.ckIntermediario.value) {// se seleccionó el checkbox
        ...
        if (idIntermediario.length > 0) {
          ...
          const newPedido = {
            ...
            difundido: (this.condicionesviaje === null ? 0: this.condicionesviaje.selectedTipoDifusion ===undefined || this.condicionesviaje.selectedTipoDifusion=== null ? 0: this.condicionesviaje.selectedTipoDifusion ),
            tipo: 2,
          ...
          this.nomencladoresService.postPedido(newPedido)
            .subscribe(data => {
              ...
              this.nomencladoresService.desglosarPedido(datadesglose)
                .subscribe(data => {
                  if (data.success) {
                    this.loader.close();
              ...
              if (this.busqueda != undefined) {
                ...
                this.centrosService.putBusqueda(this.busqueda)
                  .subscribe(data => {
                    if (data.success) {
                      this.loader.close();
                      return;
              ...
              if (this.esPrePedido) {
                ...
                this.getItemSub = this.homeService.deletePrePedido(this.data.payload.id_pre_pedido)
                  .subscribe(data => {
                    this.loader.close();
                    return;
                  },
          ...
          this.atencionService.confirm({ message: 'El Pedido no se pudo agregar, no seleccionó un Intermediario' }).subscribe(res => {
            if (res) {
              return;
            }
          });
        ...
        let valoroperador = '';
        if ((this.mostrarOperador === true) && (this.f.selectedOperador.value !== '')) {
        ...
        this.nomencladoresService.postPedido(newPedido)
          .subscribe(data => {
            ...
            this.loader.close();
            this.initValue();
            this.alertService.confirm({ message: 'Pedido retorno agregado correctamente!', tipo: 'exito' })
              .subscribe(res => {
                if (res) {
                  if (this.checkTurneada) {
                    this.addChoferesListaDisponibles(this.idPedido)
        ...
        if (idIntermediario.length > 0) {
          ...
          this.nomencladoresService.postPedido(newPedido)
            .subscribe(data => {
              ...
              this.router.navigateByUrl('/home//asignarViajeRetorno/' + this.idPedido);
              this.dialogLocaRef.close();
            },
              err => {
                this.loader.close();
      ...
      .subscribe(res => {
        ...
        this.origenesService.postOrigen(res)
          .subscribe(data => {
            ...
            this.getItemsOrigen();
            this.alertService.confirm({ message: '¡Lugar de Carga Agregado!', tipo: 'exito' })
              .subscribe(res => {
                if (res) {
                  return;
                }
            ...
            err => {
              this.loader.close();
              this.atencionService.confirm({ message: 'Este Lugar de Carga ya se encuentra ingresado.' }).subscribe(res1 => {
                if (res1) {
                  return;
                }
              });
      ...
      .subscribe(res => {
        ...
        this.centrosService.postCentroCliente(res)
          .subscribe(data => {
            ...
            this.getItemsDador();
            this.alertService.confirm({ message: '¡Cargador agregado!', tipo: 'exito' })
              .subscribe(res => {
                if (res) {
                  return;
                }
  ...
  ChangeCantidad(event) {
   ...
   /*  if (this.checkTurneada) {
      this.validTurneada = false;
      let cantChoferesDisponibles = 0;
      if (parseInt(event.target.value) <= this.choferesDisponiblesListaTurneada) {
        this.validTurneada = true;
      } else {
        this.validTurneada = false;
      }
  ...
  addElement() {
    if (this.addPedidoForm.controls['cantidad'].value > 0) {
      if (this.addPedidoForm.controls['cantidad'].value <= this.cantidadisponible) {
        if (!this.existeIntermediario(this.addPedidoForm.controls['selectedIntermediario'].value)) {
          this.cantidadisponible -= this.addPedidoForm.controls['cantidad'].value;
          ELEMENT_DATA.push(
            {
              intermediario: this.obtenerNombreInter(this.addPedidoForm.controls['selectedIntermediario'].value),
  ...
  openPopCondicionesViaje(option:boolean) {
    if (option) {
      ...
      let dialogRef: MatDialogRef<any> = this.dialog.open(CondicionesViajeComponent, {
        width: '720px',
        height: '80vh',
        disableClose: true,
        data: { title: title, payload: { tipopedido: 'retorno' } }
      });
      dialogRef.afterClosed()
        .subscribe(res => {
          if (!res) {
            ...
            if (this.tipocentro === '2') {
              this.addPedidoForm.controls['difundido'].setErrors(Validators.required);
              this.addPedidoForm.controls['difundido'].markAsDirty();
              this.alertService.confirm({ message: 'Debe realizar la difusión!' });
            }
            return;
          }
  ...
  addCondicionesViaje() {
    ...
    this.nomencladoresService.postPedidoCondiciones(this.condicionesviaje)
      .subscribe(data => {
        const vpedidoacoplados = [];
        for (let i = 0; i < this.condicionesviaje.tipo_acoplado.length; i++) {
          vpedidoacoplados.push({ id_pedido: this.idPedido, id_tipo_acoplado: this.condicionesviaje.tipo_acoplado[i] });
          this.nomencladoresService.postPedidoTipoAcoplado({
            id_pedido: this.idPedido, id_tipo_acoplado:
              this.condicionesviaje.tipo_acoplado[i]
          })
            .subscribe(data1 => {
            }, err => { });
        }
        for (
          let i = 0;
          i < this.condicionesviaje.zona_ideal.length;
          i++
        ) {
        ...
        err => {
          this.errorService.confirm({ message: 'Las condiciones del pedido no se pudo agregar, intentelo nuevamente.' })
            .subscribe(res => {
              if (res) {
                return;
              }
