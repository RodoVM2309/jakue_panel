...
@Component({
  selector: 'app-desvio-retorno',
  templateUrl: './desvio-retorno.component.html',
  styleUrls: ['./desvio-retorno.component.scss']
})
export class DesvioRetornoComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<DesvioRetornoComponent>,
  ...
  buildItemForm(item) {
    ...
    const id_destino = (item.id_destino !== null) ? item.id_destino : '';
  ...
  getDesvio(idViaje) {
    this.getItemSub = this.nomencladoresService.getDesvioViajeRetorno(idViaje)
      .subscribe(data => {
        this.desvio = data.data;
        this.id_destino = this.desvio[0].id_destino;
        this.newDesvio = false;
        this.buildItemForm(this.data.payload);
      });
  }
  getItemsEstadoViaje() {
  ...
  getItemsDestino() {
    this.getItemSub = this.nomencladoresService.getAllOrigenes()
      .subscribe(data => {
