...
@Component({
  selector: 'app-asignar-viaje-fertilizante',
  templateUrl: './asignar-viaje-fertilizante.component.html',
  styleUrls: ['./asignar-viaje-fertilizante.component.scss']
})
export class AsignarViajeFertilizanteComponent implements OnInit, OnDestroy {
 ...
 openPopupAsignarReserva(data:any = null) {
  ...
  this.reservasService.detalleReservas(detalleReserva).subscribe( resp => {
    let dialogRef: MatDialogRef<any> = this.dialog.open(AsignarReservaComponent, {
      ...
      height: '90%',
      disableClose: true,
      data: { title: 'title', payload: resp[0], isNew: 'isNew', filtroReserva: this.filtroReserva }
    });
  ...
  guardarViajes() {
    ...
    this.tempViajes.forEach(element => {
      ...
      newViaje.m = 'F';
      newViaje.id_reserva = element.id_reserva;
