...
@Component({
  selector: 'app-add-pedido-dador',
  templateUrl: './add-pedido-dador.component.html',
  styleUrls: ['./add-pedido-dador.component.scss'],
  providers: [
    {
...
export class AddPedidoDadorComponent implements OnInit {
  ...
  constructor(private centrosService: CentrosService, private origenesService: OrigenesService,
    ...
    public dialogLocaRef: MatDialogRef<AddPedidoDadorComponent>,
  ...
  ngOnInit() {
    ...
    this.addPedidoForm = new FormGroup({
      ...
      selectedZona: new FormControl('', [Validators.required]),
  ...
  selectCentro() {
    ...
    this.getItemsProductos(zcentro);


  }
  getItemsOrigen(xcentro) {
    this.loader.open('Buscando lugar de carga');
    this.getItemSub = this.nomencladoresService.getAllOrigenesCargador(xcentro)
      .subscribe(data => {
        this.loader.close();
  ...
  getItemsProductos(xcentro) {
    this.getItemSub = this.nomencladoresService.getAllProductosSelectCargador(xcentro)
      .subscribe(data => {
  ...
  postPedido() {
    const newPedido = {
      ...
      id_zona_destino: this.f.selectedZona.value,
      id_centro: parseInt(this.f.selectedCentro.value),
