...
@Component({
  selector: "app-detalle-consolidado",
  templateUrl: "./detalle-consolidado.component.html",
  styleUrls: ["./detalle-consolidado.component.scss"],
  animations: [
...
export class DetalleConsolidadoComponent implements OnInit {
  cupos: DetallesCupos[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DetalleConsolidadoComponent>,
  ...
  ngOnInit() {
    ...
    for (let index = 0; index < this.data.payload.cupos.length; index++) {
      ...
      if(this.data.title == 'En Destino'){
        temp.razonSocial = element.razonSocial_receptor;
      }else{
