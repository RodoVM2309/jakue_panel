...
import { AddSancionViajesComponent } from './add-sancion-viajes/add-sancion-viajes.component';
...
@Component({
  selector: 'app-lista-choferes-estados',
  templateUrl: './lista-choferes-estados.component.html',
  styleUrls: ['./lista-choferes-estados.component.scss']
})
export class ListaChoferesEstadosComponent implements OnInit {
  public listas: Lista[];
  ...
  getListaEstadoChoferesCentro() {
    this.getItemSub = this.centroService.getListaEstadoChoferesCentro()
      .subscribe(data => {
        this.lista_chofer = data.data;
        this.actEstadoDesc();
      });
  ...
  actEstadoDesc() {
    for (let i = 0; i < this.lista_chofer.length; i++) {
      if (this.lista_chofer[i].estado_chofer  && this.lista_chofer[i].estado_chofer != '0') {
        this.lista_chofer[i].estadodesc = "Bloqueado por el centro";
      } else {
  ...
  openPopUpEstados(data: any = {}) {
    ...
    const dialogRef: MatDialogRef<any> = this.dialog.open(AddEstadosChoferComponent, {
      width: '720px',
      disableClose: true,
  ...
  openPopUpSancionCantViaje(data: any = {}) {
    ...
    const dialogRef: MatDialogRef<any> = this.dialog.open(AddSancionViajesComponent, {
      width: '720px',
      disableClose: true,
