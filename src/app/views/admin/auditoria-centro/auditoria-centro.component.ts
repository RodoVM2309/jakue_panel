...
import { AuditoriaCentroService } from '../../../shared/services/auditoria-centro.service';
...
@Component({
	selector: 'app-auditoria-centro',
	templateUrl: './auditoria-centro.component.html',
	styleUrls: ['auditoria-centro.component.scss'],
	providers: [
		{
  ...
  export class AuditoriaCentroComponent implements OnInit {
	...
	constructor(
		private auditoriaCentroService: AuditoriaCentroService,
	...
	filterData(event?: PageEvent) {
		...
		this.auditoriaCentroService.getV3Auditorias(this.dateFrom, this.hourFrom, this.dateTo, this.hourTo, params.page,  params.per_page)
		.subscribe(
		  data => {
			this.loader.close();
			if (data.success) {
	...
	{					
		this.loader.open('Por favor espere..');			
		this.auditoriaCentroService.getV3Excel(this.dateFrom, this.hourFrom, this.dateTo, this.hourTo)
		.subscribe(
		  res => {
			this.loader.close();
	...
	buildSheet(data): void
	{
		...
		if (data.length > 0) 
			{														
				...
				this.excelService.exportAsExcelFile(array_exp, "AuditoriaCentro");  
