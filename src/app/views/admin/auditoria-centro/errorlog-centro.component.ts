...
import { ErrorLogCentroService } from '../../../shared/services/errorlog-centro.service';
...
@Component({
	selector: 'app-errorlog-centro',
	templateUrl: './errorlog-centro.component.html',
	styleUrls: ['errorlog-centro.component.scss'],
	providers: [
		{
  ...
  export class ErrorLogCentroComponent implements OnInit {
	...
	messages = {
		emptyMessage: `
		  <div>
			<span class="classname">No hay logs disponibles</span>
		  </div>
		`
	  };
	...
	constructor(
		private errorlogService: ErrorLogCentroService,
	...
	filterData(event?: PageEvent) {
		...
		this.errorlogService.getV3Auditorias(this.dateFrom, this.hourFrom, this.dateTo, this.hourTo, params.page,  params.per_page)
		.subscribe(
		  data => {
			this.loader.close();
			if (data.success) {
	...
	{					
		this.loader.open('Por favor espere..');			
		this.errorlogService.getV3Excel(this.dateFrom, this.hourFrom, this.dateTo, this.hourTo)
		.subscribe(
		  res => {
			this.loader.close();
	...
	buildSheet(data): void
	{
		...
		if (data.length > 0) 
			{														
				...
				this.excelService.exportAsExcelFile(array_exp, "ErrorLogCentro");  
