...
@Component({
  selector: 'app-vincular-operador',
  templateUrl: './vincular-operador.component.html',
  styleUrls: ['./vincular-operador.component.scss']
})
export class VincularOperadorComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<VincularOperadorComponent>,
  ...
  getOperadoresSinCentro() {
    this.centrosService.getOperadoresSinCentro()
      .subscribe(data => {
        this.operadoresList = data.data;
      });
  }
  comprobarCUIT() {
    let info_cuit = [];
    ...
    if (this.itemForm.invalid === false) {
      //this.loader.open();
      this.personasService.getOperadorCuit(this.itemForm.controls['cuit_cuil'].value.toString())
        .subscribe(data => {
          ...
          if (codigo == 0) {// no existe como persona
            this.confirmService.confirm({ message: 'No existe registro con este cuit. Desea ingresarlo como operador?' })
              .subscribe(res => {
                if (res) {
                        ...
                        data = res;
                          ...
                          if (data.success) {
                            ...
                            this.personasService.postRolPersona({
                              ...
                              this.itemForm.controls['id_operador'].setValue(data.data.id);
            ...
            if (codigo == 1) {// Existe como persona pero no como dador
              this.confirmService.confirm({ message: 'Ya existe el cuit pero no posee el rol Operador. Desea ingresarlo como Operador?' })
                .subscribe(res => {
                  if (res) {
                    this.personasService.postRolPersona({
                      id_rol: 11,
              ...
              if (info_cuit['centros'].length > 0) {
                for (let index = 0; index < info_cuit['centros'].length; index++) {
                  if (info_cuit['centros'][index].id_centro.toString() === this.idCentro) {
                    ...
                    this.alertService.confirm({ message: 'Ya existe el operador!'});
                    this.incorrectcuit = true;
                  }
              ...
              if (bandera == false) {
                this.itemForm.controls['id_operador'].setValue(info_cuit['id']);
                this.personasService.getPersonaById(id_usuario)
                  .subscribe(data => {
                    this.loader.close();
                    this.datospersona = data.data;
