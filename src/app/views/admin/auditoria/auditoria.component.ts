...
@Component({
  ...
  export class AuditoriaComponent implements OnInit {
	...
	filterData(event?: PageEvent) {
		...
		this.auditoriasService.getV3Auditorias(this.dateFrom, this.hourFrom, this.dateTo, this.hourTo, this.username, params.page,  params.per_page)
		.subscribe(
		  data => {
			this.loader.close();
			if (data.success) {
	...
	{					
		this.loader.open('Por favor espere..');			
		this.auditoriasService.getV3Excel(this.dateFrom, this.hourFrom, this.dateTo, this.hourTo, this.username)
		.subscribe(
		  res => {
			this.loader.close();
	...
	buildSheet(data): void
	{
		...
		if (data.length > 0) 
			{														
				...
				this.excelService.exportAsExcelFile(array_exp, "Auditorias");  
