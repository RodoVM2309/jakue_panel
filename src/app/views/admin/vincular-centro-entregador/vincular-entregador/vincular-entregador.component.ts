...
@Component({
  selector: 'app-vincular-entregador',
  templateUrl: './vincular-entregador.component.html',
  styleUrls: ['./vincular-entregador.component.scss']
})
export class VincularEntregadorComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<VincularEntregadorComponent>,
  ...
  comprobarCUIT() {
    ...
    if (this.itemForm.invalid === false) {
      //this.loader.open();
      this.personasService.getEntregadorCuit(this.itemForm.controls['cuit_cuil'].value.toString())
        .subscribe(data => {
          ...
          if (codigo == 0) {// no existe como persona
            this.confirmService.confirm({ message: 'No existe registro con este cuit. Desea ingresarlo como entregador?' })
              .subscribe(res => {
                if (res) {
                        ...
                        data = res;
                          ...
                          if (data.success) {
                            ...
                            this.personasService.postRolPersona({
                              ...
                              this.itemForm.controls['id_entregador'].setValue(data.data.id);
            ...
            if (codigo == 1) {// Existe como persona pero no como dador
              this.confirmService.confirm({ message: 'Ya existe el cuit pero no posee el rol entregador. Desea ingresarlo como entregador?' })
                .subscribe(res => {
                  if (res) {
                    this.personasService.postRolPersona({
                      id_rol: 8,
              ...
              if (info_cuit['centros'].length > 0) {
                for (let index = 0; index < info_cuit['centros'].length; index++) {
                  if (info_cuit['centros'][index].id_centro.toString() === this.idCentro) {
                    ...
                    this.alertService.confirm({ message: 'Ya existe el entregador!' });
                    this.incorrectcuit = true;
                  }
              ...
              if (bandera == false) {
                this.itemForm.controls['id_entregador'].setValue(info_cuit['id']);
                this.personasService.getPersonaById(id_usuario)
                  .subscribe(data => {
                    this.loader.close();
                    this.datospersona = data.data;
